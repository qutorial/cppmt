...
This example demonstrate how showing only the central information, \cc{friend} keyword and the
friend function name, and just hinting on the details (the complete friend signature), can make 
the syntax more appealing. This is an example of the ``Hide Redundant Syntax'' principle, as in the \rsec{genprinciples}.
...
Thanks to  \rg{concept} inheritance in \jbmps\ , \ref{modularity}, \ref{mpsconceptdeclaration}, the creation of a new expression 
...
Inheriting the \mpsid{OoDotOrArrowExpression} \rg{concept} from the \mpslang{Expression} 
...
described, it is not possible. Instead, \cc{this} expression has to be used. It makes inputting slower, but allows to
...
copy-constructor in the \fig{inhusage}. Instead of \mpsid{this->val} syntax, simply \cc{val} could be used in C++ text.
