...
\declsec{Projectional Editing}{projectionalediting}
...
Another approach which can be taken in the organization of an editor for a programming language is called \rg{projectionalapproach}. Projectional editors do not work with a low-level textual representation of a program, but rather with a higher level concept, \rg{ast}s.
...
Thirdly, all features, which in textual approach require parsing, can be implemented without a parser involved, because \rg{ast} is always known to the editor.
...
it would be written as a text code in the programming language, see \fig{constructor}. There can be in  principle many such textual visualizations, supporting different ways the code looks. Normally in 
...
principles in the \rpart{moreprojection}.